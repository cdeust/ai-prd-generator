...
public actor JudgeSelectionStrategy {
    ...
    /// - In native macOS/iOS: Includes Apple Intelligence
    /// - In Docker/Linux: Excludes Apple Intelligence
    /// - Parameters:
    ///   - primaryProvider: Primary AI provider configuration
    ///   - secondaryProviders: Additional provider configurations for diversity
    /// - Returns: Array of available AIProviderPort judges
    /// - Throws: AIProviderError if no judges available
    public func selectAvailableJudges(
        ...
        // 1. Always try to include primary provider
        if let primary = try? await providerFactory.createProvider(
            from: primaryProvider
        ) {
            judges.append(primary)
        }

        // 2. Try Apple Intelligence (only works on macOS/iOS)
        #if os(macOS) || os(iOS)
        if #available(macOS 26.0, iOS 26.0, *) {
            let appleConfig = AIProviderConfiguration(
                type: .appleFoundationModels
            )
            if let appleProvider = try? await providerFactory.createProvider(
                from: appleConfig
            ) {
        ...
        // 3. Add diversity: Other cloud providers
        for config in secondaryProviders {
            if let provider = try? await providerFactory.createProvider(
                from: config
            ) {
        ...
        // 4. Minimum 1 judge required
        guard !judges.isEmpty else {
            throw AIProviderError.invalidConfiguration(
                "No judges available - check AI provider configuration"
            )
        }
