...
public struct IndexRemoteRepositoryUseCase: Sendable {
    ...
    public func execute(
        ...
        let project = try await createProject(codebase: codebase, repositoryUrl: repositoryUrl, branch: branchName)
    ...
    private static func performBackgroundIndexing(
        ...
        do {
            ...
            try await indexUseCase.execute(codebaseId: codebase.id, projectId: project.id, files: filesWithContent)
            try await connRepo.updateLastSync(connectionId: connectionId, date: Date())
            try await CodebaseStatusUpdater.updateStatus(codebaseId: codebase.id, status: .completed, repository: codebaseRepo)
        } catch {
            try? await codebaseRepo.updateProjectIndexingError(projectId: project.id, error: error.localizedDescription)
            try? await CodebaseStatusUpdater.updateStatus(codebaseId: codebase.id, status: .failed, repository: codebaseRepo)
        }
    }

    private func findExistingCodebase(url: String, branch: String) async throws -> Codebase? {
        guard let existingProject = try await codebaseRepository.findProjectByRepository(url: url, branch: branch) else {
            return nil
        }
