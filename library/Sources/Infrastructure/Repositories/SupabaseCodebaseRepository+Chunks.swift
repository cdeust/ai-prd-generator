...
extension SupabaseCodebaseRepository {
    ...
    public func saveChunks(_ chunks: [CodeChunk], projectId: UUID) async throws -> [CodeChunk] {
        let records = chunks.map { mapper.chunkToRecord($0) }
        let data = try await databaseClient.insertBatch(table: "code_chunks", values: records)
        let savedRecords = try decode([SupabaseCodeChunkRecord].self, from: data)
        return savedRecords.map { mapper.chunkToDomain($0) }
    }
    ...
    public func findChunksByProject(
        ...
        let data = try await databaseClient.select(from: "code_chunks", columns: nil, filter: filter)
        let records = try decode([SupabaseCodeChunkRecord].self, from: data)
        return Array(records.dropFirst(offset).prefix(limit))
            .map { mapper.chunkToDomain($0) }
    }

    public func findChunksByFile(_ fileId: UUID) async throws -> [CodeChunk] {
        let filter = QueryFilter(field: "file_id", operation: .equals, value: fileId.uuidString)
        let data = try await databaseClient.select(from: "code_chunks", columns: nil, filter: filter)
        let records = try decode([SupabaseCodeChunkRecord].self, from: data)
        return records.map { mapper.chunkToDomain($0) }
    }
    ...
    public func deleteChunksByProject(_ projectId: UUID) async throws {
        let filter = QueryFilter(field: "codebase_id", operation: .equals, value: projectId.uuidString)
        try await databaseClient.delete(from: "code_chunks", matching: filter)
    }
}
